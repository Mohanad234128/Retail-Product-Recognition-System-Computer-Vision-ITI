...
app.py
-------
Retail Product Recognition System — public-facing showcase.

Run with:  streamlit run app.py

Expects a trained `model.h5` in the project root (see model_utils.py for
the search paths it checks). If no model file is found, the demo section
degrades gracefully with a clear "weights not found" state instead of
crashing — the rest of the page still renders.
"""
...
st.set_page_config(
    page_title="Retail Product Recognition | CV Showcase",
    page_icon="🛒",
    layout="wide",
...
with st.container():
    ...
    st.markdown(
        '<div class="section-sub">Upload a produce image or pick a sample below. '
        'Inference runs the exact preprocessing pipeline the model was trained on: '
        '100×100 resize, RGB, simple /255. rescale.</div>',
    ...
    if model is None:
        ...
        <div class="glass-card" style="border-color:#EF444455;">
          ...
          <div style="color:{ui.TEXT_DIM}; font-size:.88rem; line-height:1.6;">
            ...
            The demo can't run predictions without it — the rest of the page still works.
            <br/><br/>
...
with st.container():
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color=ui.ACCENT, width=3), marker=dict(size=8),
        fill="tozeroy", fillcolor="rgba(0,229,199,0.08)",
    ))
    fig.update_layout(
        template=None,
